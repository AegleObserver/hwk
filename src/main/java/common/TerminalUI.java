...
public class TerminalUI implements GameUiBridge {
    ...
    @Override
    public void setDemoRunning(boolean running) {
    ...
    private void toggleDemo() {
        if (!demoRunning) {
            ...
            setMessage("Info: demo " + (demoPaused ? "paused" : "resumed") + ".");
            refreshView();
        }
    }
    ...
    private void startDemo() {
        ...
        games[activeGameIndex] = currentGame().newGame(8);
        refreshView();
        setMessage("Info: demo started. Use Demo button to pause/resume.");
        demoThread = new Thread(() -> {
            try {
                ...
                demoPaused = false;
                try {
                    javax.swing.SwingUtilities.invokeLater(TerminalUI.this::refreshView);
                } catch (Exception ignored) {
                }
            }
        });
    ...
    private boolean processMove(String rawInput) {
        if (!isDemoInput(rawInput) && rejectDuringDemo()) return false;
    ...
    private static boolean isDemoInput(String rawInput) {
        return rawInput != null && "AUTO".equals(rawInput.trim().toUpperCase());
    }

    private void handleCurrentBoardFinished() {
        if (demoRunning) {
            String finishSummary = currentGame().getFinishSummary();
            String msg = "Demo finished on board " + (activeGameIndex + 1) + ".";
