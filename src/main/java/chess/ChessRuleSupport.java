...
final class ChessRuleSupport {
    ...
    static boolean isFriendlyPiece(char piece, char player) {
        if (piece == '.' || piece == 'x') return false;
        if (player == 'W') return Character.isUpperCase(piece);
        if (player == 'B') return Character.isLowerCase(piece);
        return false;
    }
