...
以下测试用例主要覆盖了游戏大厅切换、Chess 基本行棋、非法输入、将军提示、悔棋以及 Demo 阻断等核心功能。测试方式为在程序启动后按照指定步骤手动操作界面，并观察左侧棋盘状态与右侧提示信息。

| 编号 | 测试项 | 操作步骤 | 预期结果 | 实际结果 |
| --- | --- | --- | --- | --- |
| TC-01 | 启动与游戏大厅初始化 | 运行 `mvn clean compile exec:java`，进入主界面 | 正常进入游戏大厅，显示 Peace、Reversi、Minesweeper、Chess 四个入口 | 通过，界面正常加载 |
| TC-02 | Chess 正常行棋 | 在 Chess 模式输入一组合法坐标并提交 | 棋子移动成功，提示 `move accepted` | 通过，提示与棋盘变化均正确 |
| TC-03 | 起始格为空 | 选择一个没有棋子的坐标作为起点 | 系统提示起始格无棋子，并阻止本次操作 | 通过，提示信息正确 |
| TC-04 | 选择对方棋子 | 在己方回合选择对方棋子进行移动 | 系统提示该棋子属于对方，并拒绝操作 | 通过，输入被拦截 |
| TC-05 | 非法走法 | 让棋子执行不符合规则的移动 | 系统提示该移动不合法 | 通过，非法走法未被执行 |
| TC-06 | 输入格式错误 | 输入越界坐标或不符合格式的指令 | 系统提示输入无效，并给出正确输入说明 | 通过，错误输入被识别 |
| TC-07 | 将军提示 | 在对局中制造对王的直接威胁 | 界面显示 `Check : xx` 提示 | 通过，将军状态提示正常 |
| TC-08 | 悔棋功能 | 在完成一次有效落子后输入 `U` | 撤销上一步操作，棋盘恢复到上一步状态 | 通过，悔棋恢复正常 |
| TC-09 | 胜负结算 | 通过连续落子使一方吃掉对方的王 | 游戏结束并显示胜利结果 | 通过，结算正常 |
| TC-10 | Demo 阻断输入 | 开启 Demo 后在播放过程中继续输入棋步 | Demo 运行时输入被阻断，并提示当前不可操作 | 通过，输入被正确禁止 |

综合来看，本项目在当前版本中可以稳定完成多游戏切换、Chess 核心规则验证、非法输入处理、将军提示、悔棋与 Demo 播放控制等功能，测试结果满足本次实验要求。
