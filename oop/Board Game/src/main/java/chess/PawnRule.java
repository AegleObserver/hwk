...
public final class PawnRule {
    ...
    public static boolean isCaptureMove(BoardView board, int fromRow, int fromCol, int toRow, int toCol, char player) {
        if (!isLegalMove(board, fromRow, fromCol, toRow, toCol, player)) return false;
        return Math.abs(toCol - fromCol) == 1 && (toRow - fromRow == (player == 'W' ? -1 : 1));
    }
